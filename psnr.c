...
float RGBA_mse(const unsigned char* ref, const unsigned char* cmp, int w, int h, int stride, int a_calc, int upside_down)
{
    int error, offset;
    unsigned long long sum = 0;
    int ww, hh;
    int cmp_stride = stride;
    if (upside_down) {
        /* cmp是上下翻转的 */
        cmp += (h - 1) * cmp_stride;
    ...
    for (hh = 0; hh < h; ++hh) {
        ...
        for (ww = 0; ww < w; ++ww) {
            ...
            /* A */
            if (a_calc) {
                error = ref[offset + 3] - cmp[offset + 3];
                sum += error * error;
            }
    ...
    return (float)((double)sum / (double)(w * h * (a_calc ? 4 : 3)));
}
float RGBA_psnr(const unsigned char* ref, const unsigned char* cmp, int w, int h, int stride, int a_calc, int upside_down)
{
    const int L_sqd = 255 * 255;
    float mse = RGBA_mse(ref, cmp, w, h, stride, a_calc, upside_down);
    if (mse == 0.00) {
        return 100.00;        /* 两份相同数据的psnr设置为100 */
    }
    return (float)(10.0 * log10(L_sqd / mse));
